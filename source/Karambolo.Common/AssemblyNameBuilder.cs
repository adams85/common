    ...
    public sealed class AssemblyNameBuilder
    {
            ...
            for (var i = 1; i < n; i++)
                if ((index = (part = parts[i]).IndexOf('=')) > 0)
                {
                        ...
                        processorArchitectureKey.Equals(key, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (Enum.TryParse<ProcessorArchitecture>(value, ignoreCase: true, out var processorArchitecture) &&
                        ...
                        customKey.Equals(key, StringComparison.OrdinalIgnoreCase))
                    {
                        ...
                        if (nullValue.Equals(value, StringComparison.OrdinalIgnoreCase))
                            ...
                            try { Custom = StringUtils.HexStringToByteArray(value); }
                            catch (FormatException) { }
                    }
                }
        ...
        public string PublicKeyTokenString
        {
            get =>
                ...
                StringUtils.ByteArrayToHexString(PublicKeyToken);
            set => PublicKeyToken =
                value == null ? null :
                nullValue.Equals(value, StringComparison.OrdinalIgnoreCase) ? ArrayUtils.Empty<byte>() :
                StringUtils.HexStringToByteArray(value);
        }
        ...
        public string PublicKeyString
        {
            get =>
                ...
                StringUtils.ByteArrayToHexString(PublicKey);
            set => PublicKey =
                value == null ? null :
                nullValue.Equals(value, StringComparison.OrdinalIgnoreCase) ? ArrayUtils.Empty<byte>() :
                StringUtils.HexStringToByteArray(value);
        ...
        public bool HasAttributes =>
            ...
            ProcessorArchitecture != ProcessorArchitecture.None || Retargetable || ContentType != AssemblyContentType.Default || Custom != null;
        ...
        public override string ToString()
        {
            ...
            if (Custom != null)
                sb.Append(',').Append(' ').Append(customKey).Append('=').Append(Custom.Length > 0 ? StringUtils.ByteArrayToHexString(Custom) : nullValue);
