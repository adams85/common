...
namespace Karambolo.Common
{
    public class ListUtilsTest
    {
        ...
        public void RemoveAllTest()
        {
            ...
            ListUtils.RemoveAll(collection, item => item % 2 == 0);
            Assert.Equal(new[] { 5, 9 }, collection);

            ListUtils.RemoveAll(list, item => item % 2 != 0);
            Assert.Equal(new[] { 8, 10, 12 }, list);
        }
        ...
        public void ReverseTest()
        {
            ...
            ListUtils.Reverse(collection);
            Assert.Equal(new[] { 12, 10, 9, 8, 5 }, collection);

            ListUtils.Reverse(collection, 1, 3);
            Assert.Equal(new[] { 12, 8, 9, 10, 5 }, collection);

            ListUtils.Reverse(collection, 2, 2);
            Assert.Equal(new[] { 12, 8, 10, 9, 5 }, collection);

            Assert.Throws<ArgumentOutOfRangeException>(() => ListUtils.Reverse(collection, -1, 1));

            Assert.Throws<ArgumentOutOfRangeException>(() => ListUtils.Reverse(collection, 0, -1));

            Assert.Throws<ArgumentException>(() => ListUtils.Reverse(collection, collection.Count, 1));
        }

        [Fact]
        public void SortTest()
        {
            var collection = new IntCollection(Enumerable.Range(0, 1000));
            ListUtils.Sort(collection);
            Assert.Equal(Enumerable.Range(0, 1000), collection);

            collection = new IntCollection(Enumerable.Range(0, 1000).Select(i => 1000 - i - 1));
            ListUtils.Sort(collection);
            Assert.Equal(Enumerable.Range(0, 1000), collection);

            collection = new IntCollection(Enumerable.Range(0, 1000).Select(i => 1000 - i - 1));
            ListUtils.Sort(collection, intComparer);
            Assert.Equal(Enumerable.Range(0, 1000), collection);

            collection = new IntCollection(Enumerable.Range(0, 1000).Select(i => 1000 - i - 1));
            ListUtils.Sort(collection, intComparison);
            Assert.Equal(Enumerable.Range(0, 1000), collection);

            collection = new IntCollection { 20, 40, 10, 5, 78, 2 };
            ListUtils.Sort(collection, 1, 3);
            Assert.Equal(new[] { 20, 5, 10, 40, 78, 2 }, collection);

            collection = new IntCollection { 20, 40, 10, 5, 78, 2 };
            ListUtils.Sort(collection, 1, 3, intComparer);
            Assert.Equal(new[] { 20, 5, 10, 40, 78, 2 }, collection);

            Assert.Throws<ArgumentOutOfRangeException>(() => ListUtils.Sort(collection, -1, 1));

            Assert.Throws<ArgumentOutOfRangeException>(() => ListUtils.Sort(collection, 0, -1));

            Assert.Throws<ArgumentException>(() => ListUtils.Sort(collection, collection.Count, 1));
        }
